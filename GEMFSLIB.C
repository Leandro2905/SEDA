...
/* GEMFSLIB is implemented in a separate ACC. */
#if 1
MLOCAL char ipath[256];
MLOCAL char isel[256];
MLOCAL char name[256];
...
WORD fs_exinput(LPBYTE pipath, LPBYTE pisel, WORD *pbutton, LPBYTE pname)
{
	...
	LSTLCPY(ipath, pipath, sizeof(ipath));
	LSTLCPY(isel,  pisel,  sizeof(isel));
	LSTLCPY(name,  pname,  sizeof(name));
	pid = ap_find(ADDR("GEM     "));
	if (pid == -1)
	{
		return 0;	/* GEM.AES is not present */
	}
	msgbuf[0] = 100;
	msgbuf[1] = rlr->p_pid;
	msgbuf[2] = FP_OFF(ipath);
	msgbuf[3] = FP_SEG(ipath);
	msgbuf[4] = FP_OFF(isel);
	msgbuf[5] = FP_SEG(isel);
	msgbuf[6] = FP_OFF(name);
	msgbuf[7] = FP_SEG(name);

	if (wind_spb.sy_tas)	/* wm_update flag set */
	{
		wm_update(0);
		ap_rdwr(MU_SDMSG, fpdnm(NULLPTR, pid), 16, ADDR(msgbuf));
		ap_rdwr(MU_MESAG, fpdnm(NULLPTR, rlr->p_pid), 16, ADDR(result));
		wm_update(1);
	}
	else
	{
	...
	/* Sort through messages until we find the one containing the result.
	 * Those we don't process (repaints) we repost. */
	while (result[0] != 101)
	{
		ap_rdwr(MU_SDMSG, fpdnm(NULLPTR, rlr->p_pid), 16, ADDR(result));
		ap_rdwr(MU_MESAG, fpdnm(NULLPTR, rlr->p_pid), 16, ADDR(result));
	}
	LSTCPY(pipath, ipath);
	LSTCPY(pisel,  isel);
