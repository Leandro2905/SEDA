	...
	WORD		prets[];
{
	...
	REG CQUEUE	*pc;
/*
 * Now that accessories can be loaded into EMS, we have to use a local evbuff,
 * just like the MULTIAPP version of the AES. Otherwise we could be using the
 * address of accessory B's buffer, when accessory A is paged in. 
 */
	WORD		evbuff[8];
#if MULTIAPP
//	WORD		evbuff[8];
...
//	    m.qpb_buf = mebuff;
//#endif
	    //TRACE("Entering iasync(MU_MESAG)\r\n");
