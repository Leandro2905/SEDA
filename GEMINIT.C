...
//	 TRACE("centis=$"); ophex(centis); TRACE("\r\n$"); TRKEY();
...
//	 TRACE("gl_boxtick=$"); ophex(gl_boxtick); TRACE("\r\n$"); TRKEY();
}
	...
	VOID		(*routine)(VOID);
{
	...
	ldaddr = (LPBYTE) routine;
						...
						/*   create process to	*/
						/*   execute it		*/
	return( pstart((BYTE *)routine, pname, ldaddr, 0, 0) );
}
...
#if SINGLAPP
MLOCAL void create_mcb(LPBYTE addr, BYTE *progname)
{
/* Format of MCB:
 *  Byte 0: 'j' if not last in chain, 'e' if it is. (in a real DOS MCB,
 *  M and Z respectively. DOS Plus also uses 'm' and 'z' for memory blocks
 *  outside the DOS arena proper).
 *  Bytes 1-2: GEM PID (in a real DOS MCB, the owning PSP) 
 *  Bytes 3-4: Size of block, paragraphs 
 *  Bytes 5-6: Debug info (currently EMS paging state)
 *  Byte 7: Spare
 *  Bytes 8-15: Owner name */
	addr[0] = 'e';
	addr[1] = curpid; addr[2] = 0;

	EMS_PageMap(0, NULL, D.g_emsmap);
	addr[5] = D.g_emsmap[0];	
	addr[6] = D.g_emsmap[1];	
	addr[7] = 0;
	LBCOPY(addr+8, ADDR(progname), 8);
}
...
*
*	paccroom is in paragraphs for low memory, 16k pages for EMS.
*/
VOID sndcli(REG BYTE *pfilespec, UWORD *paccroom, BOOLEAN in_ems)
{
	REG WORD	handle;
	WORD		err_ret;
	LPBYTE		ldaddr;
...
#if DEBUG
	  TRACE("PGMLD: $");
	  for (n = 0; D.s_cmd[n]; n++) TRCHAR(D.s_cmd[n]); 
	  TRKEY();
#endif
	  if (in_ems)
	  {
	  	err_ret = pgmld_e(handle, &D.s_cmd[0], &ldaddr, (WORD *)paccroom);
		if (err_ret != -1) create_mcb(MK_FP(gl_emm_seg, 0), &D.s_cmd[0]);
	  }
	  else	err_ret = pgmld  (handle, &D.s_cmd[0], &ldaddr, (WORD *)paccroom);
#if DEBUG
	  ophex(err_ret);
	  TRCHAR(' ');
	  ophex((long)ldaddr >> 16);
	  TRCHAR(':');
	  ophex((long)ldaddr & 0xFFFF);
	  TRACE("\r\n$");
	  TRKEY();
#endif
	  dos_close(handle);
...
/* If the ACC was loaded into EMS, it will now be in the page frame. Create
 * a process to execute it*/
	  if (err_ret != -1)
	  {
	    pstart((BYTE *)gotopgm, pfilespec, ldaddr, in_ems, err_ret);
	  }
	}
}
...
*	0x33800
*/
MLOCAL WORD acc_count = 0;

VOID ldaccs_l(WORD rsid)
{
	REG WORD	i;
	WORD		ret;
	ULONG		laccroom;
	...
	strcpy(rs_str(rsid), &D.g_dir[0]);
	dos_sdta(ad_dta);
	ret = TRUE;

	for(i = acc_count ; (i < NUM_ACCS) && (accroom > 0) && (ret); i++)
	{
#if DEBUG
	  TRACE("Search for $");
	  for (n = 0; ad_path[n]; n++) TRCHAR(ad_path[n]);
	  TRACE(" $");
	  TRKEY();
#endif
	  ret = (first) ? dos_sfirst(ad_path, F_RDONLY) : dos_snext();
	  if (ret)
	  {
	    first = 0;
#if DEBUG
	    TRACE("Loading $");
	    for (n = 0; gl_dta[n+30]; n++) TRCHAR(gl_dta[n+30]);
	    TRACE("\r\n$");
	    TRKEY(); 
#endif
	    sndcli(&gl_dta[30], &accroom, 0);
	    ++acc_count;
	  }
#if DEBUG
	  else TRACE("\r\n$");
...
// Load accessories into EMS memory.
VOID ldaccs_e(WORD rsid)
{
	REG WORD	i;
	WORD		ret;
	UWORD		eaccroom = EMS_Avail();
	int 		first = 1;
#if DEBUG
	int n;
#endif
	...
	for(i = acc_count ; (i < NUM_ACCS) && (eaccroom > 0) && (ret); i++)
	{
#if DEBUG
	  TRACE("Search for $");
	  for (n = 0; ad_path[n]; n++) TRCHAR(ad_path[n]);
...
/* Detect EMS */
	...
	if (!gl_ems_avail)
	{
		static LPBYTE warning = "No expanded (EMS) memory manager "
			  "was found.\r\nThis version of GEM requires EMS."
			  "\r\n$";
		dos_func(0x900, (ULONG)warning);
		return;
	}
	gl_emm_mapsize = EMS_PageMap(3, NULL, NULL);
	D.g_emsmap    = dos_alloc( (NUM_PDS + 3) * gl_emm_mapsize);	
	
	for (i = 4; i > 0; i--)
	{
		D.g_emshandle = EMS_Alloc(i);
		if (D.g_emshandle) break;
	}
	if (i == 0)
	{
		static LPBYTE warning = "No expanded (EMS) memory is available."
			  "\r\nThis version of GEM requires EMS."
			  "\r\n$";
		dos_func(0x900, (ULONG)warning);
		return;
	...
	// Work out how many windows and ORECTs we can have. Allow 10 ORECTs
	// per window.
	wdw_storage = sizeof(WINDOW) + 10 * sizeof(ORECT) + sizeof(OBJECT);

	// Work out how many windows would fit in an EMS page, and then
	// multiply by number of pages. Doing it the other way (multiply
	// page count by 16k and then divide by wdw_storage) would be more
	// accurate but involve long arithmetic.
	NUM_WIN   = (16384 / wdw_storage) * gl_emm_size;
	...
	NUM_ORECT = NUM_WIN * 10;
	D.w_emmwin   = MK_FP(gl_emm_seg, 0);
	D.g_emmtree  = MK_FP(gl_emm_seg, NUM_WIN * sizeof(WINDOW));
	D.g_emmolist = MK_FP(gl_emm_seg, NUM_WIN * (sizeof(WINDOW) + sizeof(OBJECT)));
#if DEBUG
	TRACE("NUM_WIN=$");
	ophex(NUM_WIN);
	TRACE("\r\n$");
...
//	TRACE("0\r\n$"); TRKEY();
	...
	else
	{
	  ...
	  if (totpds > 2)
	  {
		ldaccs_e(STAES);	/* First, load *.AES accessories */
		ldaccs_e(STACC);	/* Then *.ACC accessories */
		ldaccs_l(STALC);	/* Finally, *.ALC - accessories which 
					 * don't work in EMS */
	  }
#endif
						/* fix up icons		*/
