...
MLOCAL WORD w_kind(WORD wh)
{
	ems_get_window(wh, &D.w_win0);
	return  D.w_win0.w_kind;
}
...
//	TRACE("ct_msgup: calling ap_sendmsg\r\n");
...
VOID hctl_window(REG WORD w_handle, WORD mx, WORD my)
{
	GRECT		t, f, pt;
	WORD		x, y, w, h, ii;
	WORD		kind;
	...
	ems_get_window(gl_wtop,  &wtop);
	ems_get_window(w_handle, &wcur);	

	if ( (w_handle == gl_wtop) ||
	     ( (wcur.w_flags & VF_SUBWIN) && (wtop.w_flags & VF_SUBWIN) )  )
	{
						/* went down on active	*/
						/*   window so handle	*/
						/*   control points	*/
...
doelev:		message = (cpt == W_HELEV) ? WM_HSLID : WM_VSLID;
	...
	{
#if SINGLAPP
	  ct_msgup(WM_UNTOPPED, wtop.w_owner, gl_wtop, x, y, w, h);
	...
	ct_msgup(message, wcur.w_owner, w_handle, x, y, w, h);
}

VOID hctl_rect(WORD key)
{
	WORD		title, item;
	REG WORD	mesag;
	REG PD		*owner;
	...
	if ( gl_mntree != 0x0L )
	{
	  ...
	  if ( mn_do(&title, &item, key) )
	  {
	    ...
	    if ( title == THEDESK )
	    {
//	      TRACE("System menu activated: item "); ophex(item); TRACE("\r\n");
	      if (item > 2)
	      {
		item -= 3;
	        owner = desk_ppd[item];
//	        TRACE("owner="); ophex(owner); TRACE("\r\n");
#if FIXLATER
	/* use w, new mnlib	 */
		...
		if (gl_wtop >= 0 )
		{
		  ems_get_window(gl_wtop, &D.w_win0);
		  ct_msgup(WM_UNTOPPED, D.w_win0.w_owner, gl_wtop, 0, 0, 0, 0);
	...
	switch( cpt )
	  {
	    ...
	    case BACKTAB:
		ems_get_window(gl_wtop, &D.w_win0);
 	        ct_msgup(WM_UNTOPPED, D.w_win0.w_owner, gl_wtop, x, y, w, h);
 ...
 *
 *  [JCE] This is ViewMAX-specific, of course. 
 */     
		for (i = 4; i; i--)
		{
			ems_get_window(i, &D.w_win0);
			open[i] = D.w_win0.w_flags & VF_INTREE;
		}
...
/* end of changes KH 920107 */
			...
			} while( !(D.w_win0.w_flags & VF_INUSE) && (w_handle != 1) );
			if ((w_handle == 1) && open[3])
				...
				ems_get_window(w_handle, &D.w_win0);
			    } while( !(D.w_win0.w_flags & VF_INUSE) && (w_handle != 2) );
		}
		else if (cpt == TAB)
		    switch (w_handle)
		    {
...
/* end of changes KH 920107 */
	...
	ems_get_window(w_handle, &D.w_win0);
	ct_msgup(message, D.w_win0.w_owner, w_handle, x, y, w, h);
}
