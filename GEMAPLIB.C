...
*/
MLOCAL VOID old_ap_rdwr(WORD code, REG PD *p, WORD length, LPVOID pbuff)
{
	QPB		m;
#if DEBUG
	TRACE("ap_rdwr: code="); ophex(code);
	TRACE(" p="); ophex(p);
	TRACE(" length="); ophex(length);
	TRACE("\r\n$");
#endif
						/* do quick version if	*/
						/*   it is standard 16	*/
	...
	if ( (code == MU_MESAG) &&
	     ...
	     (length == 16) )
	{
//	  TRACE("ap_rdwr: Quick peek\r\n$");
	  LBCOPY(pbuff, p->p_qaddr, p->p_qindex);
	  p->p_qindex = 0;
	}
	else
	{
//	  TRACE("ap_rdwr: Queueing message\r\n$");
	  m.qpb_ppd = p;
	  m.qpb_cnt = length;
	  m.qpb_buf = pbuff;
...
/* Do the same sort of trick that MULTIAPP GEM does to pass messages: 
 * keep a local buffer and decant to/from it. This allows messages to
 * be sent between ACCs loaded in EMS memory, using a 16-byte 
 * intermediate buffer. */
VOID ap_rdwr(WORD code, REG PD *p, WORD length, LPVOID pbuff)
{
	WORD tbuff[8];
	WORD chunk;
	LPBYTE rbuff = (LPBYTE)pbuff;

	while (length)
	{
		chunk = min(16, length);
		LBCOPY(tbuff, rbuff, chunk);
		old_ap_rdwr(code, p, chunk, tbuff);
		LBCOPY(rbuff, tbuff, chunk);
		length -= chunk;
		rbuff += chunk;
	}
}
