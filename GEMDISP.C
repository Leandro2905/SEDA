...
VOID show_ems(char c, UDA *uda)
{
#if DEBUG
	int n;
	LPWORD w = (LPWORD)(uda->u_ems);
	LPWORD w2 = MK_FP(0xE000, 6);
...
/* Save EMS state. If there isn't one, allocate it. */
	if (!rlr->p_uda->u_ems)
	{
        	rlr->p_uda->u_ems = D.g_emsmap + (gl_emm_mapsize * (1 + rlr->p_pid));
#if DEBUG
		TRACE("s: Creating new EMS map at $");
		ophex(rlr->p_uda->u_ems);
		TRACE("\r\n$");
#endif
	}
	if (rlr->p_uda->u_ems)
	{
		EMS_PageMap(0, NULL, rlr->p_uda->u_ems);
		//show_ems('s', rlr->p_uda);
	}
 ...
 */
	...
	/* Restore EMS context, if there is one */
	if (rlr->p_uda->u_ems)
	{
		//show_ems('r', rlr->p_uda);
		EMS_PageMap(1, rlr->p_uda->u_ems, NULL);
	}
